<x-layout bodyClass="g-sidenav-show bg-gray-100" title="Tambah Kriteria Seminar Proposal">
    <x-navbars.sidebar activePage='proposal-criteria'></x-navbars.sidebar>

    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg">
        <x-navbars.navs.auth titlePage="Tambah Kriteria Seminar Proposal"></x-navbars.navs.auth>
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    ...
                                    <p class="text-white text-sm mb-0 opacity-8">
                                        Tambahkan kriteria penilaian baru untuk seminar proposal
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div class="card-body p-4">
                            <form method="POST" action="{{ route('proposalCriterias.store') }}">
                                @csrf
                                <div class="row">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-select @error('type') is-invalid @enderror"
                                                ...
                                                <option value="material"
                                                    {{ old('type') == 'material' ? 'selected' : '' }}>
                                                    Material
                                                </option>
                                                <option value="presentation"
                                                    {{ old('type') == 'presentation' ? 'selected' : '' }}>
                                                    Presentation
                                                </option>
                                            </select>
                                            @error('type')
